...
class MessengerContext extends _Context.default {
  constructor({
    ...
    _defineProperty(this, "_personaId", void 0);
  ...
  async sendSenderAction(action, options) {
    if (!this._session) {
    ...
    this._isHandled = true;
    const args = [this._session.user.id, action, _objectSpread({
      access_token: this._customAccessToken,
      persona_id: this._personaId
    }, options)];
  ...
  async typingOn(options) {
    ...
    this._isHandled = true;
    const args = [this._session.user.id, _objectSpread({
      access_token: this._customAccessToken,
      persona_id: this._personaId
    }, options)];
